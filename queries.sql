...
-- Query 3: Receivers who have claimed the most food
...
-- Query 4: Total quantity of food available
SELECT SUM(Quantity) AS Total_Available_Quantity
FROM food_listings;

-- Query 5: City with the highest number of food listings
SELECT Location, COUNT(*) AS Total_Listings
FROM food_listings
GROUP BY Location
ORDER BY Total_Listings DESC;

-- Query 6: Most commonly available food types
SELECT Food_Type, COUNT(*) AS Count
FROM food_listings
GROUP BY Food_Type
ORDER BY Count DESC;

-- Query 7: Number of claims made per food item
...
-- Query 8: Provider with the most successful claims
...
-- Query 9: Percentage of claims by status
SELECT Status,
       COUNT(*) AS Count,
...
-- Query 10: Average quantity of food claimed per receiver
...
-- Contact info of providers by city (used in app filter)
